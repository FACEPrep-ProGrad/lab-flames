...
public class FlamesCheck extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * Uncomment the below code to test your output */
		  ...
		  if (k == 'f') {
			  RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/friends.jsp"); rd.forward(request, response);
		  
		  } 
		  else if (k == 'l') { 
			  RequestDispatcher rd=this.getServletContext().getRequestDispatcher("/WEB-INF/views/lovers.jsp"); rd.forward(request, response);
